...
$test = ["2xxxxoneightasd45xxx"];


function getFirstAndLastDigitsFromString(string $string): string
{
    $replaceArray = [
        "one" => 1,
        "two" => 2,
    ...
    preg_match_all('/(?=(one|two|three|four|five|six|seven|eight|nine))/', $string, $matches, PREG_OFFSET_CAPTURE);
    $result = $matches[1];

    foreach ($result as $element) {
        $indexes[$element[1]] = $replaceArray[$element[0]];
    }
    //print_r($indexes);

    // get all numbers from string
    preg_match_all('/[0-9]/', $string, $matches, PREG_OFFSET_CAPTURE);
    $result = $matches[0];
    foreach ($result as $element) {
        $indexes[$element[1]] = $element[0];
    }

    ksort($indexes);

    $firstNumer = array_shift($indexes);
    $lastNumer = $firstNumer;
    if ($indexes) {
        $lastNumer = array_pop($indexes);
    }
    $finalNumber = $firstNumer . $lastNumer;

    return $finalNumber;
}

function getValuesFromCords(array $list): array
{
    $listOfValues = [];
    foreach ($list as $element) {
        $finalNumber = getFirstAndLastDigitsFromString($element);
        $listOfValues[] = $finalNumber;
    }
    return $listOfValues;
}

//$returnArray = getValuesFromCords($test);
$returnArray = getValuesFromCords($cords);
//$returnArray = getValuesFromCords($testCords);
$returnSum = array_sum($returnArray);
