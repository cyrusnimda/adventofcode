...
function getEngineFromString(array $engine): array
{
    $arrayEngine = array_map(function ($value) {
        return str_split($value);
    }, $engine);
    return $arrayEngine;
}

function getAdjacent(array $engine, int $x, int $y): array
{
    $cords_to_check = [];

    $cords_to_check[] = [$x - 1, $y - 1];
    $cords_to_check[] = [$x - 1, $y];
    $cords_to_check[] = [$x - 1, $y + 1];

    $cords_to_check[] = [$x, $y - 1];
    $cords_to_check[] = [$x, $y + 1];
    ...
    $adjacent = [];
    foreach ($cords_to_check as $cords) {
        if (isset($engine[$cords[0]][$cords[1]])) {
            if (is_numeric($engine[$cords[0]][$cords[1]])) {
                $number = getNumberFromCords($engine, $cords[0], $cords[1]);
                if (!in_array($number, $adjacent)) {
                    $adjacent[] = $number;
                }
            }
        }
...
function getNumberFromCords(array $engine, int $x, int $y): int
{
    $originalY = $y;
    $number = $engine[$x][$y];
    if (!is_numeric($number)) {
        throw new Exception("Not a number in cords $x, $y");
    }
    // Fill left numbers
    while ($y > 0 && is_numeric($engine[$x][--$y])) {
        $number = $engine[$x][$y] . $number;
        //print($number . "\n");
    }

    // Fill right numbers
    $y = $originalY;
    while ($y < (sizeof($engine[$x]) - 1) && is_numeric($engine[$x][++$y])) {
        //print("$x, $y --> ");
        $number = $number . $engine[$x][$y];
        //print($number . "\n");
    }
...
function getEngineNumber(array $engine): int
{
    $total = 0;
    $arrayEngine = getEngineFromString($engine);
    //print_r($arrayEngine);
    foreach ($arrayEngine as $x => $row) {
        foreach ($row as $y => $element) {
            if ('*' === $element) {
                $adjacent = getAdjacent($arrayEngine, $x, $y);
                $is_gear = (sizeof($adjacent) == 2);
                if ($is_gear) {
                    $multiply = $adjacent[0] * $adjacent[1];
                    $total += $multiply;
                }
            }
        }
